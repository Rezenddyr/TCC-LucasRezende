...
def extrai_params(ind):
    return (ind[0],  ind[1],  ind[2],  ind[3],
            ...
            int(ind[8]),  int(ind[9]),  int(ind[10]), int(ind[11]),
...
def cria_individuo():
    ...
    for _ in range(N_SIZE):    ind.append(r.uniform(MIN_N,    MAX_N))
    for _ in range(VMAX_SIZE): ind.append(r.uniform(MIN_VMAX, MAX_VMAX))
    for _ in range(IND_SIZE):  ind.append(r.uniform(MIN_STRATEGY, MAX_STRATEGY))
    return ind
...
def main(seed):
    ...
    for gen in range(10001):
        ...
        if gen % 100 == 0:
            ...
            print(f"GEN: {gen}\nIndividuo: \n{[extrai_params(POPULACAO[mi])]}\nAPTIDAO: \n{mv}")
            with open(arq_res, "a") as f:
                f.write(f"GEN: {gen}\nIndividuo: \n{list(extrai_params(POPULACAO[mi]))}\nAPTIDAO: \n{mv}\n")

    mv = min(APTIDAO); mi = APTIDAO.index(mv)
    tt = time.time() - inicio
    msg = (f"\n--- RESULTADO FINAL ---\nErro (norma-1): {mv}\n"
           f"Parâmetros: {list(extrai_params(POPULACAO[mi]))}\n"
