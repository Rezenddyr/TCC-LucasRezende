...
def get_bounds(indx):
    ...
    if indx < 25:   return MIN_K,    MAX_K     # k   (15)
    if indx < 40:   return MIN_N,    MAX_N     # n   (15)
    return MIN_VMAX, MAX_VMAX                   # vmax (10)
...
def twoBody(y, t, p):
    ...
    ydot = np.empty((10,))
    ydot[0] = (vmax[0] * (1.0 - H(AJ, N[9])) - N[0]) / tau[0]
    ydot[1] = (vmax[1] * H(BE, N[4]) - N[1]) / tau[1]
    ydot[2] = (vmax[2] * sum6(H(CB, N[1]), H(CF, N[5]), H(CA, N[0])) - N[2]) / tau[2]
    ydot[3] = (vmax[3] * H(DF, N[5]) - N[3]) / tau[3]
    ydot[4] = (vmax[4] * (1.0 - H(EJ, N[9])) - N[4]) / tau[4]
    ydot[5] = (vmax[5] * H(FA, N[0]) - N[5]) / tau[5]
    ydot[6] = (vmax[6] * sum6(H(GB, N[1]), H(GF, N[5]), H(GA, N[0])) - N[6]) / tau[6]
    ydot[7] = (vmax[7] * H(HF, N[5]) - N[7]) / tau[7]
    ydot[8] = (vmax[8] * H(IG, N[6]) * H(IH, N[7]) - N[8]) / tau[8]
    ydot[9] = (vmax[9] * H(JI, N[8]) - N[9]) / tau[9]
    return ydot
