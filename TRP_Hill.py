...
def twoBody(y, t,
            ...
            kAA, kAC, kAD, kAE, kBA, kBC, kBD, kBE, kCD, kCE, kDC, kEA, kEB, kEC, kEE,
            nAA, nAC, nAD, nAE, nBA, nBC, nBD, nBE, nCD, nCE, nDC, nEA, nEB, nEC, nEE):
    ...
    hAA = hill(NA, nAA, kAA);  hAC = hill(NC, nAC, kAC)
    hAD = hill(ND, nAD, kAD);  hAE = hill(NE, nAE, kAE)
    hBA = hill(NA, nBA, kBA);  hBC = hill(NC, nBC, kBC)
    hBD = hill(ND, nBD, kBD);  hBE = hill(NE, nBE, kBE)
    hCD = hill(ND, nCD, kCD);  hCE = hill(NE, nCE, kCE)
    hDC = hill(NC, nDC, kDC)
    hEA = hill(NA, nEA, kEA);  hEB = hill(NB, nEB, kEB)
    hEC = hill(NC, nEC, kEC);  hEE = hill(NE, nEE, kEE)

    ydot = np.empty((5,))
    ydot[0] = (((1-hAA)*(1-hAD)*(1-hAE)
                + (1-hAA)*(1-hAC)*hAE
                + hAA*hAD*(1-hAE)) - NA) / tauA
    ydot[1] = (((1-hBA)*(1-hBD)*(1-hBE)
                + (1-hBA)*(1-hBC)*hBE
                + hBA*hBD*(1-hBE)) - NB) / tauB
    ydot[2] = ((hCD * hCE) - NC) / tauC
    ydot[3] = (hDC - ND) / tauD
    ydot[4] = ((hEA * hEB * hEC + hEE) - NE) / tauE
    return ydot
...
def plota_resultados(ind, pasta, seed):
    ...
    for idx, (pred, orig, nome) in enumerate([(pA, A_ORIGINAL, 'g1'), (pB, B_ORIGINAL, 'g2'),
                                               (pC, C_ORIGINAL, 'g3'), (pD, D_ORIGINAL, 'g4'),
                                               (pE, E_ORIGINAL, 'g5')]):
        ax = axs[idx // 3][idx % 3]
